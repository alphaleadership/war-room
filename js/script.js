document.addEventListener('DOMContentLoaded', function () {
  if (document.getElementById('map') && typeof conflicts !== 'undefined' && conflicts.length > 0) {
    ...
    const conflictMarkers = new ConflictMarkers(map);

    conflicts.forEach(conflict => {
      conflictMarkers.addMarker(conflict);
    });
  }

  if (document.getElementById('news-banner') && typeof newsItems !== 'undefined' && newsItems.length > 0) {
    const newsBannerConfig = themeConfig.news_banner || {};
    if(newsBannerConfig.enable) {
      new NewsBanner('news-banner', newsItems, newsBannerConfig);
    }
  }
});
